...
function extractNameFromTemplate(value) {
  return value.replace('!', '').split(' ').filter((el, id) => id > 0).join(' ');
}
...
function encodeToRot13(str) {
  return str.split('').map((el) => (el.charCodeAt(0) < 65 || el.charCodeAt(0) > 122 ? el : String.fromCharCode(el.charCodeAt(0) + (el.toLowerCase() < 'n' ? 13 : -13)))).join('');
}
